...
export const InitialOrderScreen = ({
  ...
  useEffect(() => {
    if (selectedShippingCarrier === null && orderQuote?.shipping_options) {
      const lowest = orderQuote.shipping_options.reduce((min, curr) =>
  ...
  const lowestShippingCarrierCost = orderQuote?.shipping_options?.reduce((min, curr) =>
    curr.cost < min.cost ? curr : min,
    orderQuote?.shipping_options[0]
  ).cost || 0;
  ...
  return (
    <div className="rf-max-w-lg rf-mx-auto rf-bg-white rf-rounded-2xl rf-py-8 rf-flex rf-flex-col rf-gap-3">
      <h2 className="rf-text-3xl rf-font-bold rf-text-center">
        Order PCB
      </h2>
      ...
      {orderQuote?.is_completed &&
        ...
        !orderQuote?.error && (
          <VendorQuoteCard
            ...
            lowestShippingCarrierCost={lowestShippingCarrierCost + orderQuote.total_cost_without_shipping}
          />
        )}
