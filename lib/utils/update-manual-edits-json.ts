...
export const updateManualEditsJson = (
  event: EditPcbComponentLocationEvent,
  props: RunFrameProps,
  circuitJson: AnyCircuitElement[],
  lastFsMapRef: React.MutableRefObject<Map<string, string> | null>,
) => {
  ...
  const pcb_component = circuitJson?.filter(
    (c: any) =>
      ...
      c.pcb_component_id === event.pcb_component_id,
  ) as PcbComponent[]
  ...
  const source_component = circuitJson?.filter(
    (c: any) =>
      ...
      c.source_component_id === pcb_component[0].source_component_id,
  ) as SourceComponentBase[]

  if (!source_component?.length) {
    console.error(
      "Source component not found for PCB component:",
      pcb_component[0],
    )
    return
  }
